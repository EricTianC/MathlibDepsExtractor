...
def getAllConstsFromConst (n : Name) : TermElabM (Array Name) := do
  ...
  let set := HashSet.insertMany (∅: HashSet Name) res
  return set.toArray
...
-- def filterStringAsFileName (s : String) : String := -- 优化 replace 逻辑
--   (s.toList.filter (fun c => c.isAlphanum || c == '.')).toString
...
def serializeAndWriteToFileInDir (source : Source) (depth : Nat) (dir : String): TermElabM Bool := do
  let consts ← getConstsFromSource source
  ...
  let g ← getUsedConstantGraph consts depth
  let js ←  serializeList g
  let _ ← writeJsonToFile (dir ++ "/" ++ ((toString name).append ".json")) js
  return true
...
-- #eval serializeAndWriteToFile (Source.Namespace "Mathlib") 3
-- set_option pp.rawOnError true
-- #eval getConstsFromSource (Source.Namespace "Real")
